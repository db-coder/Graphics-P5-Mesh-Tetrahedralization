...
class pts // class for manipulaitng and displaying pointclouds or polyloops in 3D 
 ...
 void savePts(String fn) 
  ...
  pts addTube(pt A, pt B)
  {
    boolean flag = false;
    for(int i = 0; i < nv - 1; i+=2)
    {
      if((A.x == G[i].x) && (A.y == G[i].y) && (A.z == G[i].z) && (B.x == G[i+1].x) && (B.y == G[i+1].y) && (B.z == G[i+1].z))
      {
        flag = true;
        break;
      }
  ...
  int findTube(pt A, pt B){
    int flag = -1;
    for (int i = 0; i < nv-1; i+=2){
      if(((A.x == G[i].x) && (A.y == G[i].y) && (A.z == G[i].z) && (B.x == G[i+1].x) && (B.y == G[i+1].y) && (B.z == G[i+1].z))||((B.x == G[i].x) && (B.y == G[i].y) && (B.z == G[i].z) && (A.x == G[i+1].x) && (A.y == G[i+1].y) && (A.z == G[i+1].z))){
        // tube A-B or B-A is in the array
        flag = i;
      }
    }
    return flag;
